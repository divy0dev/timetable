...
import { Subject, Faculty, TimeSlot, TimetableData } from './types';

function App() {
  const [subjects, setSubjects] = useState<Subject[]>([]);
  const [faculty, setFaculty] = useState<Faculty[]>([]);
  const [timeSlots, setTimeSlots] = useState<TimeSlot[]>([]);
  const [lecturesPerSubject, setLecturesPerSubject] = useState<number>(0);
  const [lunchBreak, setLunchBreak] = useState<TimeSlot | null>(null);
